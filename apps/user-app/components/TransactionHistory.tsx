...
export default function TransactionHistory({ transactions }: { transactions: Transaction[] }) {
    ...
    const filteredTransactions = transactions
        .filter((txn) => { //Filter Logic
            if (filter === "SENT" && ((txn.type === "P2P" && txn.direction !== "SENT") || txn.type === "ON_RAMP")) {
                return false;
            }
            if (filter === "RECEIVED" && txn.type === "P2P" && txn.direction !== "RECEIVED") {
                return false;
            }
